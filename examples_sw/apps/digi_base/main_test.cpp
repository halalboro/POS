...
constexpr auto const defMinSize = 32 * 1024;
...
int main(int argc, char *argv[])  
{
    ...
    for(int i = 0; i < n_reps; i++) {
        input_buffers[i] = (uint32_t*) cthread[0]->getMem({CoyoteAlloc::HPF, n_pages_host});
        output_buffers[i] = (uint32_t*) cthread[0]->getMem({CoyoteAlloc::HPF, n_pages_rslt});
        ...
        // Clear output buffer
        memset(output_buffers[i], 0, outputSize2);
